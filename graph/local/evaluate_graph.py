...
async def run_evaluation():
    ...
    EVAL_SESSION_ID = "system_quality_eval_session"
    successful_probes = 0
    
    for idx, probe in enumerate(probes, 1):
        print(f"Firing Probe #{idx}: '{probe}'")
        try:
            # 🎯 FIX: Added session_id and dataset constraints to protect your metadata
            res = await cognee.recall(
                query_text=probe, session_id=EVAL_SESSION_ID, datasets=[DATASET])
            res_str = str(res).lower()
            
            if "no provided context" in res_str or len(res_str.strip()) < 40:
                print(f"  ❌ Probe Status: Disconnected path.")
            else:
                print(f"  ✅ Probe Status: Path active.")
                successful_probes += 1
        except Exception as e:
            print(f"  ❌ Probe Status: Execution error ({e})")
